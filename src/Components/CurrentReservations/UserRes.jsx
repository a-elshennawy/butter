...
export default function CurrentReservations() {
  ...
  const [CurrRes, setCurrRes] = useState(
    ResDetails ? JSON.parse(ResDetails) : []
  );
          ...
          <div className="booked row">
            {CurrRes.length > 0 ? (
              CurrRes.map((reservation, index) => (
                <div key={index} className="reservation-card col-lg-3 col-10">
                  <h5>reservation #{index + 1}</h5>
                  <p>
                    <strong>guest full name:</strong> {reservation.FullName}
                  </p>
                  <p>
                  ...
                  {reservation.additional_notes && (
                    <p>
                      <strong>notes:</strong> {reservation.additional_notes}
                    </p>
                  )}
                  <button
                    className="cancel"
                    onClick={() => {
                      handleCancellation(index);
                    }}
                  >
                    cancel reservation
                  </button>
